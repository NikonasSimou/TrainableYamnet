...
def yamnet_frames_model(params):
  ...
  Function was slightly modified by https://github.com/NikonasSimou so as to pack  a batch of 
  waveforms in order to do transfer learning of batches of data.
  """
  # waveform = layers.Input(batch_shape=(None,), dtype=tf.float32) 
  waveform = layers.Input(batch_shape=(1, None)) #  EXPLAIN, modified in order to accept batch of wavs as input
  waveform_padded = features_lib.pad_waveform(tf.squeeze(waveform, axis=0), params) #squeeze batch of waveforms
